...
namespace eve::algo
{
  ...
  struct reduce2_ : TraitsSupport
  {
    using traits_type = typename TraitsSupport::traits_type;

    template <typename Op, typename Op1, typename Zero, typename Wide>
    struct delegate
    {
      Op op;
      Op1 op1;
      ...
      delegate(Op op, Op1 op1, Zero zero, Wide init) : op(op), op1(op1), zero(zero) {
    ...
    template <typename Rng, typename Op,  typename Op1, typename Zero, typename U>
    EVE_FORCEINLINE U operator()(Rng&& rng, std::pair<Op, Zero> op_zero, Op1 op1, U init) const
    {
      ...
      delegate<Op, Op1, Zero, wide_t> d{op_zero.first, op1, op_zero.second, init_as_wide};
    ...
    {
      return operator()(EVE_FWD(rng), std::pair{eve::add, eve::zero}, eve::add, init);
    }
    ...
    {
      return operator()(EVE_FWD(rng), op_zero, op_zero.first, init);
    }
  };

  inline constexpr auto reduce2 = function_with_traits<reduce2_>[default_simple_algo_traits];
}
